...
if __name__ == "__main__":
    ...
    # "deprel_b": 10,
    # "postag_b": 11,
    with open('./sourceData/data_engineering/train_eda_t_ratio.csv','r',encoding='utf-8') as f:
        for (index,sample) in enumerate(f):
           line = sample.strip().split('\t')
           if len(line) != 13:
            print(len(line))
           # word_a = eval(line[4])
           # deprel_a = eval(line[6])
           # postag_a = eval(line[7])
           # if len(word_a) != len(deprel_a) or len(word_a) != len(postag_a) or len(postag_a) != len(deprel_a):
           #     print(line)
